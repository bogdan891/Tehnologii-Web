...
application.post('/', async (request, response, next) => {
  try {
    ...
    for (let u of request.body) {
      const university = University.create(u);
      for (let s of u.students) {
        const student = Student.create(s);
        university.addStudent(student);
        registry[s.key] = student;
      }
      for (let c of u.courses) {
        const course = Course.create(c);
        registry[c.key] = course;
        university.addCourse(course);
      }
      for (let e of u.enrollements) {
        registry[e.coursetKey].addCourse(registry[e.studentKey]);
        await registry[e.coursetKey].save();
      }
    }
    response.sendStatus(204);
  } catch (error) {
...
application.get('/', async (request, response, next) => {
  try {
    ...
    const exportData = universities.map((u) => {
      const universitiesJSON = u.toJSON();
      const students = universitiesJSON.Students || [];
      const courses = universitiesJSON.Courses || [];
      const enrollements = [];
      students.forEach((s) => {
        s.Courses.forEach((c) => {
          enrollements.push({
